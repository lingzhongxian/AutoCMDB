...
#         else:
#             return HttpResponse("系统配置出错！")

def hello(request):
    response=render(request,"hello.html")
    response.set_signed_cookie("kkkkkk","vvvvvv",salt="test......")
    return response
...
class AssetView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
            for i in  ['#4']:
                data_dict = dic[i]
                models.Disk.object.create(**data_dict)
